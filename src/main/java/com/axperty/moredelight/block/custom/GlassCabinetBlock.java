...
public class GlassCabinetBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof GlassCabinetBlockEntity) {
                ItemScatterer.spawn(world, pos, ((GlassCabinetBlockEntity) blockEntity).getDroppableInventory());
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof GlassCabinetBlockEntity) {
                GlassCabinetBlockEntity glasscabinetBlockEntity = (GlassCabinetBlockEntity) blockEntity;
                boolean newOpenState = !glasscabinetBlockEntity.isOpen();
                glasscabinetBlockEntity.setOpen(newOpenState);
