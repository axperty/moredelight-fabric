...
public class GlassCabinetBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    ...
    public Text getDisplayName() {
        return Text.translatable("container.moredelight.glass_cabinet_block");
    }
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
        if (!player.getEntityWorld().isClient) {
            player.getEntityWorld().playSound(null, pos, SoundEvents.BLOCK_WOODEN_TRAPDOOR_OPEN, SoundCategory.BLOCKS, .5f, 1.0f);
        }
        return new GlassCabinetBlockScreenHandler(syncId, inv, this, this.propertyDelegate) {
            @Override
            public void onClosed(PlayerEntity player) {
                super.onClosed(player);
                if (!player.getEntityWorld().isClient) {
                    player.getEntityWorld().playSound(null, pos, SoundEvents.BLOCK_WOODEN_TRAPDOOR_CLOSE, SoundCategory.BLOCKS, .5f, 1.0f);
                }
                assert world != null;
    ...
    private void onServerTick() {
        if (ticksUntilClose > 0) {
            ...
            if (ticksUntilClose == 0) {
                ...
                if (currentState.getBlock() instanceof GlassCabinetBlock) {
